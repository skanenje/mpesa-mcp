...
// registerMpesaPrompts registers all M-Pesa prompts with the MCP server
func registerMpesaPrompts(server *mcp.Server) {
	// STK Push prompt
	server.AddPrompt(
		&mcp.Prompt{
		...
		func(ctx context.Context, req *mcp.GetPromptRequest, args map[string]string) (*mcp.GetPromptResult, error) {
